# Gold-Ore Skill 质量审查流程

> 本文档定义 gold-ore skill 的质量闭环机制，遵循 memory/skill-quality-loop.md 标准。
...
gold-ore 采用 **A + F** 组合（最低配置）：
- **模式 A**：多视角并行审查（2 个 Reviewer）
- **模式 F**：问题 severity 分级

如果项目为 Tier 1 级别（>100 t Au）或用户明确要求高质量，升级为 **A + C + F**：
...
当用户任务同时涉及 gold-ore 和其他 skill 时：
1. **Cu-Au 共生矿床**（Porphyry Au-Cu、IOCG-Au）：调用 copper-ore 的知识库
2. **通用框架**：mineral-system-eval 负责五要素框架评分
3. **质量审查**：gold-ore 的 Reviewer-1 + mineral-system-eval 的 Reviewer 共同完成
