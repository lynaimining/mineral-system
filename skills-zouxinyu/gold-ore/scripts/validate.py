...
# coding: utf-8
"""
validate.py -- gold-ore skill report validator
Usage:
...
def validate_report(report_path):
    ...
    if len(paras) < MIN_PARAGRAPHS:
        issues.append(f"too few paragraphs ({len(paras)} < {MIN_PARAGRAPHS}), content may be truncated")
    ...
    if hits:
        issues.append(f"unreplaced placeholders found: {hits}")

    fig_count = len(doc.inline_shapes)
    if fig_count == 0:
        warnings.append("no embedded figures detected (ignore if text-only report)")
    elif size_kb < 1000 and fig_count > 3:
        warnings.append(f"{fig_count} figures present but file is small ({size_kb:.0f} KB) -- images may be low-res")
